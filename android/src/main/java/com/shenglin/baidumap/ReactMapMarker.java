...
public class ReactMapMarker {
    ...
    public void buildMarker(ReadableMap annotation) throws Exception {
        ...
        if (annotation.hasKey("data")) {
            ReadableMap datamap = annotation.getMap("data");
            Bundle mybundle = new Bundle();
            JSONObject jsono = RNUtils.readableMapToJson(datamap);
            mybundle.putString("markerData", jsono.toString());
            options.extraInfo(mybundle);
        }
        if (annotation.hasKey("animateType")) {
            String type = annotation.getString("animateType");
            if (type.equals("grow")) {
    ...
    private int getDrawableResourceByName(String name) {
        int source = this.mContext.getResources().getIdentifier(name, "drawable", this.mContext.getPackageName());
